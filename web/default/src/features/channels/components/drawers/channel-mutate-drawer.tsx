...
export function ChannelMutateDrawer({
  ...
  const baseUrlFieldDisabled = sensitiveLocked || !baseUrlInputEnabled
  ...
  const applyConnectionInfo = useCallback(
    (connectionInfo: ChannelConnectionInfo) => {
      ...
      form.setValue('base_url', baseUrlInputEnabled ? connectionInfo.url : '', {
        shouldDirty: true,
        shouldValidate: true,
      })
  ...
  const createModeFetcher = useCallback(async (): Promise<string[]> => {
    ...
    const response = await fetchModels({
      ...
      base_url: baseUrlInputEnabled ? form.getValues('base_url') || '' : '',
    })
    if (response.success && response.data) {
      return response.data
    }
    throw new Error(response.message || 'No models fetched from upstream')
  }, [baseUrlInputEnabled, canEditSensitive, form, t])
                                ...
                                <FormField
                                  ...
                                  render={({ field }) => (
                                    <FormItem>
                                      ...
                                      <FormDescription>
                                        {!baseUrlInputEnabled
                                          ? t('Custom API address has been disabled by the administrator')
