...
export default function SettingsLog(props) {
  ...
  function onSubmit() {
    const updateArray = compareObjects(inputs, inputsRow).filter(
      (item) => item.key !== 'historyTimestamp' && item.key !== 'operationLogTimestamp',
    );
  ...
  async function onCleanOperationLog() {
    if (!inputs.operationLogTimestamp) {
      showError(t('请选择日志记录时间'));
      return;
    }

    const now = dayjs();
    const targetDate = dayjs(inputs.operationLogTimestamp);
    const targetTime = targetDate.format('YYYY-MM-DD HH:mm:ss');
    const currentTime = now.format('YYYY-MM-DD HH:mm:ss');
    const daysDiff = now.diff(targetDate, 'day');
    ...
    Modal.confirm({
      ...
      content: (
        <div style={{ lineHeight: '1.8' }}>
          ...
          >
            ...
            <Text style={{ color: '#333' }}> {t('之前的所有操作日志')}</Text>
          </div>
          <p style={{ marginTop: '12px' }}>
            <Text type='danger'>
              {t('此操作不可恢复，请仔细确认时间后再操作！')}
      ...
      onOk: async () => {
        try {
          setLoadingCleanOperationLog(true);
          const res = await API.delete(
            `/api/operation_log/?target_timestamp=${Date.parse(inputs.operationLogTimestamp) / 1000}`,
          );
          const { success, message, data } = res.data;
          if (success) {
            showSuccess(`${data} ${t('条操作日志已清理！')}`);
            return;
          } else {
            throw new Error(t('操作日志清理失败：') + message);
          }
        } catch (error) {
          showError(error.message);
        } finally {
          setLoadingCleanOperationLog(false);
        }
      },
    });
  }
  ...
  useEffect(() => {
    ...
    currentInputs['operationLogTimestamp'] = inputs.operationLogTimestamp;
            ...
            <Row gutter={16}>
              ...
              <Col xs={24} sm={12} md={8} lg={8} xl={8}>
                <Spin spinning={loadingCleanOperationLog}>
                  <Form.DatePicker
                    label={t('清除历史操作日志')}
                    field={'operationLogTimestamp'}
                    type='dateTime'
                    inputReadOnly={true}
                    onChange={(value) => {
                      setInputs({
                        ...inputs,
                        operationLogTimestamp: value,
                      });
                    }}
                  />
                  <Text
