...
func TestHandleGroupRatioUpdatesTieredSettlementAfterGroupRetry(t *testing.T) {
	...
	common.SetContextKey(ctx, constant.ContextKeyMultiGroup, "retry")
	...
	require.Equal(t, 2.0, groupRatioInfo.GroupRatio)
	require.Equal(t, 2.0, info.TieredBillingSnapshot.GroupRatio)
	require.Equal(t, 200, result.ActualQuotaAfterGroup)
}
